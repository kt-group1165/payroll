...
export function ReconciliationContent({
  ...
  const updateFilter = (companyId: string, month: string) => {
    ...
    router.push(`/billing/reconciliation${qs ? `?${qs}` : ""}`);
  };
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <label className="text-sm">法人
          <select className="ml-2 border rounded px-2 py-1 text-sm bg-background"
            value={selectedCompanyId} onChange={(e) => updateFilter(e.target.value, filterMonth)}>
            {companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
        </label>
        <label className="text-sm">請求月
          <select className="ml-2 border rounded px-2 py-1 text-sm bg-background"
            value={filterMonth} onChange={(e) => updateFilter(selectedCompanyId, e.target.value)}>
        ...
        <button className="text-xs text-blue-600 hover:underline" onClick={() => router.refresh()}>
          🔄 再集計
        </button>
      </div>
