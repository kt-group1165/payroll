...
export type OfficeLite = { id: string; office_number: string; name: string; short_name: string; company_id: string | null };
...
export function BillingContent({
  ...
  selectedOfficeNum: string;
  selectedMonth: string;
  monthColumns: string[];
  rows: TableRow[];
}) {
  const router = useRouter();
  // ─── client-side filter 状態 ─────────────────────────
  ...
  // server fetch を駆動する filter は URL params で更新
  const updateUrl = (companyId: string, officeNum: string, month: string) => {
    const params = new URLSearchParams();
    if (companyId) params.set("company", companyId);
    if (officeNum) params.set("office", officeNum);
      ...
      <div className="border rounded-md p-3 mb-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            value={selectedMonth}
            onChange={(e) => updateUrl(selectedCompanyId, selectedOfficeNum, e.target.value)}
          >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => updateUrl(e.target.value, "", selectedMonth)}
          >
          ...
          <select
            ...
            onChange={(e) => updateUrl(selectedCompanyId, e.target.value, selectedMonth)}
          >
