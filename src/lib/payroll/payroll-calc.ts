...
export type HourlyPayroll = {
  ...
  legacy_used?: string[];
  /**
   * ★ 出勤簿も 旧システムの日計も 無く、訪問実績などから **推定** した項目。2026-09-26
   *
   * 出勤時間の出どころは 3 段ある (実測 3,807 人月):
   *   tier1 出勤簿 (payroll_attendance_records)        670 人月 (17.6%)
   *   tier2 旧システムの日計で代替                    2,926 人月 (76.8%)  → legacy_used に出る
   *   tier3 どちらも無く 訪問実績等から推定             211 人月 ( 5.5%)  → ★ 何にも出ていなかった
   * tier3 は legacy_used に立たないので 画面からも検証からも見えない「隠れフォールバック」だった。
   * 実測: 総括表と突合できた 130 人月のうち 44 人月 (33.8%) が 1% 超ズレ、差額計 ¥1,113,518。
   * ★ tier2 は「旧システムの値をそのまま使った」= 根拠があるが、tier3 は **当方の推定**。
   *   同じ「旧」バッジに混ぜると 根拠の強さが違うものが同じに見えるので 分けて出す。
   */
  estimated_used?: string[];
...
export function employeeWorkMinutes(
  ...
  // ⚠ 旧システムの日計は 研修・会議の時間を 既に含んでいる (KT姉崎 渡邉 2026-05: 日計 9,620 = 総括 / HRD研修 60分込み)。
  //   ここに研修を足すと二重になる (2026-09-21 に一度足して 出勤時間のずれが 70→90 人月に増えた)
  if (legacyWorkMin != null && legacyWorkMin > 0) return legacyWorkMin;
  // 日計が無いとき (旧システム停止後) は 事業所書式の研修・会議・HRD研修・初任者研修の時間も足す (2026-09-21)。
  //   総括表 2026-03〜07 (出勤簿なし・訪問 + 移動全量 で推定した場合): 時給 1,836人月 68.2% → 86.8% (+341 / 悪化 0) /
  //   月給 537人月 61.8% → 74.1% (+68 / 悪化 2)
