 ...
 *
 * 出どころ: 総括表 xlsm の印字シートの「社会保険」列 (1 = 加入)。
 *   ⚠ これは旧システムが持っている **登録事実** であって 計算結果ではない。
 *     当方に月ごとの社保情報が他に無いため ここから取る。
 *   ⚠ 兼務の人は 拠点別の行 (氏名に _<拠点>) を飛ばし、_合計 の行を使う。
 *
...
const rows = [];
let seen = 0, same = 0, noEmp = 0, noCol = 0;
for (const m of MONTHS) {
  const p = join(S, `soukatsu${m}`, "extract.json");
  if (!existsSync(p)) { console.warn(`⚠ 総括表が無い: ${p}`); continue; }
  for (const f of JSON.parse(readFileSync(p, "utf8"))) {
    const onum = OFFNUM[f.office];
    if (!onum) continue;
    for (const r of f.rows) {
      const num = nn(r._code);
      if (!num) continue;
      if (!("社会保険" in r)) { noCol++; continue; }
      const name = String(r["氏名"] ?? "");
      if (name.includes("_") && !name.includes("_合計")) continue;   // 兼務の拠点別行
      if (!emp.has(`${onum}|${num}`)) { noEmp++; continue; }
      seen++;
      const si = Number(r["社会保険"]) === 1;
      ...
      rows.push({ office_number: onum, employee_number: num, processing_month: m,
        ...
        _name: name.replace("_合計", ""), _office: f.office, _from: eff, _src: cur === undefined ? "職員マスタ" : "月ごと" });
    }
  }
}
console.log(`総括表の「社会保険」列: ${seen} 人月 / 既に一致 ${same} / ★ 直す ${rows.length}`);
if (noEmp) console.log(`  当方に職員が居ない: ${noEmp} 人月 (対象外)`);
if (noCol) console.log(`  「社会保険」列が無いシート: ${noCol} 行`);
const per = new Map();
for (const r of rows) {
