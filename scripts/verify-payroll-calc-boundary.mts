...
// 旧システムは時間を小数で持つので 1/3 時間系は .5 に届かず切り下がる (2026-09-21 総括表 558人月で検算)
eq("★ 勤続手当(時給): 1,385分 × 90円/h = 2,077.5 ではなく 2,077 円 (23.083333h として計算する)",
  computeTenureAllowance(true, 240, "時給", "訪問介護", 1385, 0, 0), 2077);
eq("★ 勤続手当(時給): 4,815分 (= 80.25h ちょうど) × 30円/h = 2,407.5 は 2,408 円 (切り下げない)",
  computeTenureAllowance(true, 60, "時給", "訪問介護", 4815, 0, 0), 2408);
eq("★ 勤続手当(時給): 3,070分 × 90円/h = 4,605 ちょうどは 4,605 円のまま",
  computeTenureAllowance(true, 240, "時給", "訪問介護", 3070, 0, 0), 4605);
...
eq("出勤時間: 旧日計は研修込みなので足さない (二重計上しない)", employeeWorkMinutes(0, 0, 0, 0, 9000, 90), 9000);
{
