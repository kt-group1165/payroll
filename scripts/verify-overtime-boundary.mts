...
// ── screenAttendanceToVisitRecords (画面入力の出勤簿 → 訪問介護の給与計算の形。2026-09-18) ──
{
  ...
  eq("日残業・週残業は空 (給与計算は 勤務時間−8h を使う = Excel 取込と同じ)", [one({ start_time: "09:00", end_time: "19:00", break_minutes: 60 }).overtime_daily, one({ start_time: "09:00", end_time: "19:00", break_minutes: 60 }).overtime_weekly], ["", ""]);
  ...
  // 月をまたぐ週: 前月の日も渡すが、返すのは対象月の日だけ
  const cross = ["2026-06-29","2026-06-30","2026-07-01","2026-07-02"].map((d) => row({ work_date: d, start_time: "09:00", end_time: "18:00", break_minutes: 60 }));
  eq("月をまたぐ週: 7 月の日だけ返す", screenAttendanceToVisitRecords(cross, emps, "2026-07").map((r) => r.day), [1, 2]);
  // ★ 負のコントロール: 残業を出さない = 8h 超の日は 勤務時間 に 9:00 が入り、給与計算側で 1:00 が残業になる
  eq("★★ 9h 勤務の日は 勤務時間 9:00 (給与計算で 1h が残業になる材料が残っている)", one({ start_time: "09:00", end_time: "19:00", break_minutes: 60 }).work_hours, "9:00");
}
